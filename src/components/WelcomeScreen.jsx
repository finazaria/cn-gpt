...
import { Building2, TrendingUp, AlertTriangle, Package, BarChart2, Calendar, Network, Lock } from 'lucide-react';
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: 28, maxWidth: 580 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-2)', lineHeight: 1.7, marginBottom: 10 }}>
          Your AI-powered customer intelligence agent. Uncover portfolio insights,
          trends, and opportunities — across all your clients.
        </p>
        <div style={{
          ...
          🔵 Now piloting for Business Banking · Corporate & Commercial Segments
        </div>
      ...
      <div style={{
        ...
        maxWidth: 740, width: '100%', marginBottom: 20,
      }}>
        {CARD_DEFINITIONS.map((card, i) => {
          const Icon = ICON_MAP[card.icon] || Building2;
          const enabled = cardConfig[card.key] !== false;
          return (
            <button key={card.key} onClick={() => enabled && onSend(card.prompt(company.name))}
              disabled={!enabled}
              style={{
              ...
              onMouseEnter={e => { if (enabled) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 24px rgba(0,0,0,0.08)'; e.currentTarget.style.borderColor = card.color; }}}
              onMouseLeave={e => { if (enabled) { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.borderColor = 'var(--border)'; }}}
            >
              {!enabled && (
                <div style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 600, color: enabled ? 'var(--text-1)' : 'var(--text-3)', marginBottom: 4 }}>
                {card.label}
              </div>
